"""Counterpart to the API slurper; takes description files generated by the API slurper and builds Python code to pop into the client object."""
import json
import os
...
if __name__ == "__main__":
    ...
    for api_func in funcs:
        ...
        if len(py_parameter_list) > 0:
            if len(py_parameter_list) <= 3 and True not in [
                p.endswith(" = Omit") for p in py_parameter_list
            ]:
        ...
        # Generate the docstr
        docstr = ""
        has_desc = func["description"] is not None and len(func["description"]) > 0
        ...
        if has_desc or has_args:
            ...
            if (
                docstr.find("\n") == -1
            ):  # Single line docstrings should be all on one line
                docstr = '"""{}"""\n'.format(docstr)
            else:
                docstr = '"""\n{}\n"""\n'.format(docstr)
        ...
        if len(url_params) > 0:
            request_string += (
                ".format("
                + ",".join([f["name"] + "=" + f["name"] for f in url_params])
                + ")"
            )
    ...
    def {title}(self{py_params}){result}:
        ...
        if func["category"] != prev_category:
            ...
            print(
                "\n{fence}\n# {category} #\n{fence}".replace("\n", "\n    ").format(
                    category=prev_category, fence=fence
                )
            )
        print(result)

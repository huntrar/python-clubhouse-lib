...
def readAPIDoc(doc:str) -> Documentation:
    ...
    def feed_title(line:str, stripped:str):
        ...
        if len(split) == 2:
            if (split[0].strip() == 'Code' and split[1].strip() == 'Description') or split[0].strip().isdigit():
...
if __name__ == "__main__":
    print("""This is the API slurper. It is designed to accept snippets of the Clubhouse API documentation that have been copied from the website to the clipboard.
            ...
            try:
                ...
                try:
                    with open(fname, 'r') as f:
                        ...
                        do_write = False
                        if doc != other:
                            replace = input('"{}" already exists under category "{}". Replace? (Y/n):'.format(doc['title'], other['category']))
                            if replace.lower() == 'y' or replace == '':
                                do_write = True
                            else:
                                print('Err: Unrecognized option', replace)
                        else:
                            print('"{}" already exists and is identical to the provided information. Skipping...'.format(doc['title']))
                except json.JSONDecodeError:
                    print('"{}" already exists but appears to be corrupted, so it will be overwritten.'.format(doc['title']))
